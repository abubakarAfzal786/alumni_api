...
class UserController extends Controller
{
    ...
    public function udpateUserActivity(Request $request)
    {
        $user = $request->get('user');
        $newRequest=$request->all();
        $newRequest['id']=$request->id;
        // if ($request->video_games_fav_title) {
        //     $newRequest['video_games_fav_title'] = serialize($request['video_games_fav_title']);
        // }
        // if ($request->athletic_stuff_you_play) {
        //     $newRequest['athletic_stuff_you_play'] = serialize($request->athletic_stuff_you_play);
        // }

        $user =  UserActivity::where('id', $user->id)->get();
        $user->parents_from_alma = $request->parents_from_alma;
        $user->siblings_from_alma = $request->siblings_from_alma;
        $user->play_video_games = $request->play_video_games;
        $user->military_rank = $request->military_rank;
        $user->military_code = $request->military_code;
        $user->video_games_frequency = $request->video_games_frequency;
        $user->video_games_categories = $request->video_games_categories;
        $user->video_games_fav_title = $request->video_games_fav_title;
        $user->athletic_stuff_you_play = $request->athletic_stuff_you_play;
        $user->have_exotic_pet = $request->have_exotic_pet;
        $user->havePet = $request->havePet;
        $user->pet = $request->pet;
        $user->fan_of_alma_football = $request->fan_of_alma_football;
        $user->fan_of_alma_basketball = $request->fan_of_alma_basketball;
        $user->in_us_military = $request->in_us_military;
        $user->military_type = $request->military_type;
        $user->dependent_us_military_person = $request->dependent_us_military_person;
        $user->instrument = $request->instrument;
        $user->long_have_lived_here = $request->long_have_lived_here;
        $user->country_to_travel = $request->country_to_travel;
        $user->state_to_travel = $request->state_to_travel;
        $user->city_to_travel = $request->city_to_travel;
        $user->uid = $user->id;
        $user->save();
        ...
        // if ($user->video_games_fav_title) {
        //     $user->video_games_fav_title = unserialize($user->video_games_fav_title);
        // }
        // if ($user->athletic_stuff_you_play) {
        //     $user->athletic_stuff_you_play = unserialize($user->athletic_stuff_you_play);
        // }
    ...
    {
        $user = $request->get('user');
        $userActivity =  UserActivity::where('id', $user->id)->first();
        ...
        return response()->json(['user' => $userActivity]);
    }
