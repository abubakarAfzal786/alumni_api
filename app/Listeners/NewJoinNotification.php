            ...
            if (!is_null($alumniTokens)) {
                ...
                if (count($tokenArr) > 0) {
                    ...
                    if ($alumniWorkCareer !== '') {
                        ...
                        $msg = $user->first_name.' '.$user->last_name.' ('.$alumni->match.'%)'.' is a ('.$userWorkCareers->work_title.') like you! Shake hands! 💼 ';
                        $this->sendMultiPush(3, $tokenArr, $title, $msg, $user);
                    }
                }
                    if ($matchOrgName !== '') {
                        $title = 'Other user joins that is from same student organization';
                        $msg = $user->first_name.' '.$user->last_name.' ('.$alumni->match.'%)'.' was in '.$matchOrgName.' like yOU! Say Hi!';
                        $this->sendMultiPush(6, $tokenArr, $title, $msg, $user);
                    }
                    if ($matchAthleteName !== '') {
                        $title = 'Other user joins that is from same student athletic';
                        $msg = $user->first_name.' '.$user->last_name.' ('.$alumni->match.'%)'.' played '.$matchAthletesName.' like yOU! High Five!';
                        $this->sendMultiPush(6, $tokenArr, $title, $msg, $user);
                    }
                    if ($alumni->match > 29) {
                        $title = $user->first_name.' '.$user->last_name.' just joined.';
                        $msg = $user->first_name.' '.$user->last_name.' ('.$alumni->match.'%)'.' just joined AlumniMatch!';
